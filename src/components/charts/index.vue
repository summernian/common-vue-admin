<template>
	<div class="custom-component-chart">
		<a-spin :spinning="loading">
			<!-- 图表实例 -->
			<div v-if="!isNoData && !isFailed" class="custom-component-chart__instance" :id="localId"
				:style="{height: localHeight}"></div>
			<!-- 处理结果 -->
			<div v-else class="custom-component-chart__result">
				<template v-if="isNoData">
					<slot name="noData">
						<div class="custom-component-chart__resultTitle">{{('common.noData')}}</div>
					</slot>
				</template>
				<template v-else-if="isFailed">
					<slot name="falied">
						<div class="custom-component-chart__resultTitle">{{('common.failed')}}</div>
					</slot>
				</template>
			</div>
		</a-spin>
