...
const service = axios.create({
	baseURL: configure.mock ? '/mock' : configure.VUE_APP_BASE_API,
	// 请求超时时间，15秒
	timeout: 15 * 1000, 
	// 跨域时使用第三方的cookie
...
service.interceptors.response.use(
	response => {
		...
		if (response.status === 200) {
			if(!res.code) {
				// 某些接口不返回code，直接返回data
				return Promise.resolve(res);
			} else if(res.code == 200) {
				return Promise.resolve(res);
			} else {
				console.log(res, 'promise reject');
	...
	error => {
		...
		if(!error.response){
			...
			if(['401', '402', '403', '404'].includes(error.response.status.toString())) {
				console.log('报错地址: ', error.response.config.url);
				message.error(`错误地址：${error.response.config.url}，状态码：${error.response.status}，来自后端给的错误，去battle吧！`, 5);
			} else {
